...
```plaintext
# วาง output ที่นี่ ![alt text](image-2.png)
...
```plaintext
# บันทึก headers และคำอธิบายที่นี่ X-Content-Type-Options: nosniff

ป้องกัน: การโจมตีแบบ MIME Sniffing

อธิบาย: บังคับให้ Browser เชื่อประเภทไฟล์ตามที่ Server แจ้งเท่านั้น ป้องกันคนร้ายหลอกส่งสคริปต์อันตรายมาในรูปแบบไฟล์ภาพ

X-Frame-Options: SAMEORIGIN

ป้องกัน: การโจมตีแบบ Clickjacking

อธิบาย: ห้ามไม่ให้เว็บไซต์อื่นดึงหน้าเว็บของเราไปใส่ใน <iframe> เพื่อหลอกให้ผู้ใช้กดปุ่มที่มองไม่เห็น

Content-Security-Policy (CSP)

ป้องกัน: การโจมตีแบบ XSS (Cross-Site Scripting)

อธิบาย: กำหนดขอบเขตว่า Browser สามารถโหลด Script หรือทรัพยากรจากแหล่งใดได้บ้าง เพื่อป้องกันการรันโค้ดแปลกปลอมที่ถูกฉีดเข้ามา

Strict-Transport-Security (HSTS)

ป้องกัน: การโจมตีแบบ Man-in-the-Middle (MITM)

อธิบาย: บังคับให้ Browser เชื่อมต่อผ่านโปรโตคอล HTTPS ที่ปลอดภัยเท่านั้น

Referrer-Policy: no-referrer

ป้องกัน: การรั่วไหลของข้อมูลใน URL

อธิบาย: ไม่ส่งข้อมูล URL ต้นทาง (Referrer) ไปยังเว็บไซต์อื่นเมื่อผู้ใช้คลิกลิงก์ออกไป เพื่อความเป็นส่วนตัวและความปลอดภัย
...
Continuous Integration (CI): คือการ "ตรวจสอบ" โค้ดอัตโนมัติ (Build & Test) ทุกครั้งที่ Push เพื่อป้องกันบั๊ก

Continuous Deployment (CD): คือการ "ส่งมอบ" โค้ดขึ้น Server จริงอัตโนมัติเมื่อตรวจสอบผ่านแล้ว

ตัวอย่างจาก Workflow ในแล็บ
CI: ขั้นตอนที่ระบบสั่ง npm install และรัน Newman Tests เพื่อเช็คว่า API ทำงานถูกต้อง (สีเขียวถึงจะไปต่อได้)

CD: ขั้นตอนการสั่งให้ Self-hosted runner อัปเดตโค้ดและรันแอปเวอร์ชันใหม่ให้ผู้ใช้เข้าถึงได้ทันที
...
```plaintext
# 1. ข้อดี (Pros)
Performance: Vercel กระจาย Frontend ไปยัง Edge Network ทั่วโลก ทำให้เว็บโหลดเร็วมาก

Scalability: แยกทรัพยากรออกจากกัน ถ้าหน้าเว็บคนเข้าเยอะแต่ Backend งานน้อย เราก็ขยายเฉพาะ Frontend ได้ (ประหยัดค่าใช้จ่าย)

Specialization: แต่ละ Platform ถูกออกแบบมาเพื่อหน้าที่เฉพาะ (Vercel เก่งเรื่อง Static/React, Render เก่งเรื่อง Server/Database)

Independent Deployment: แก้โค้ดหน้าบ้าน ก็ Deploy ใหม่ได้โดยไม่กระทบหลังบ้าน ลดความเสี่ยงระบบล่ม

2. ข้อเสีย (Cons)
Latency: ข้อมูลต้องวิ่งข้าม Server คนละที่กัน อาจทำให้ช้าลงเล็กน้อย (Network Overhead)

Complexity: ต้องจัดการเรื่อง CORS (การอนุญาตให้คนละ Domain คุยกัน) และต้องดูแล URL สองชุด
...
```plaintext
# 1. ความแตกต่างระหว่าง Newman CI และ Smoke Tests
Newman CI Tests (Pre-Deployment): รันในขั้นตอน Build/Test บนเครื่องจำลอง (Runner) เพื่อเช็คว่าโค้ดที่เขียนมาใหม่ทำงานถูกต้องตาม Logic หรือไม่ ก่อน จะอนุญาตให้เอาไปขึ้น Server จริง

Post-Deploy Smoke Tests: รันบน Production Server (Server จริงที่ผู้ใช้เข้าถึง) ทันทีหลัง Deploy เสร็จ เพื่อเช็คว่าระบบโดยรวม (เช่น การเชื่อมต่อ Database, สิทธิ์การเข้าถึงไฟล์) ยังทำงานได้ปกติในสภาพแวดล้อมจริง

2. ทำไมต้องมีทั้งสองแบบ?
เพราะ "โค้ดผ่าน ไม่ได้แปลว่าระบบรันได้" * CI ช่วยกรองบั๊กจากตัวโค้ด (Software Bugs)

Smoke Tests ช่วยเช็คความผิดพลาดจากสภาพแวดล้อม (Infrastructure/Environment Issues)

3. ตัวอย่างสถานการณ์ที่ CI ผ่าน แต่ Smoke Tests ล้มเหลว
สถานการณ์: การตั้งค่า Environment Variable (เช่น Database Password) ผิดพลาดบน Production Server

ทำไม CI ถึงผ่าน: ในขั้นตอน CI ระบบใช้ฐานข้อมูลจำลอง (Mock/Test DB) ซึ่งรันได้ปกติ โค้ดทำงานถูกต้อง Newman จึงให้ผ่าน

ทำไม Smoke Tests ถึงล้มเหลว: เมื่อ Deploy ไปยัง Server จริง (Production) ตัวแปร DB_PASSWORD ดันพิมพ์ผิด ทำให้ Backend เชื่อมต่อฐานข้อมูลจริงไม่ได้ หน้าเว็บจึงดึงข้อมูลห้องพักไม่ได้ (ขึ้น Error) ทั้งที่ตัวโค้ดไม่ได้บั๊ก
...
```plaintext
# ตอRuntime: ป้องกันขณะแอปกำลังรันและมีผู้ใช้ใช้งานจริง

CI Pipeline: ป้องกันตั้งแต่ขั้นตอนเขียนโค้ดและก่อนจะส่งมอบงาน เพื่อไม่ให้ช่องโหว่หลุดรอดไปถึงมือผู้ใช้บที่นี่
...
```plaintext
# เหตุผลที่ต้องใช้ GitHub Secrets แทนการเขียนลงไปตรงๆ ในไฟล์ YAML มีประเด็นสำคัญดังนี้ครับ:

1. ทำไมต้องใช้ GitHub Secrets?
ความปลอดภัย (Security): Secrets จะถูกเข้ารหัส (Encrypted) และไม่ถูกแสดงผลใน Log ของการรัน Workflow (จะขึ้นเป็น ***)

การจัดการสิทธิ์ (Access Control): เฉพาะผู้ที่มีสิทธิ์จัดการ Repository เท่านั้นที่เข้าไปแก้ไขค่าได้ แต่ไม่สามารถ "ดู" ค่าเดิมที่ใส่ไปแล้วได้

ความยืดหยุ่น (Flexibility): เปลี่ยนค่าได้ง่ายโดยไม่ต้องแก้โค้ดหรือทำการ Push ใหม่ เช่น เมื่อต้องการเปลี่ยนรหัสผ่านฐานข้อมูล

2. ถ้าใส่ JWT_SECRET ตรงๆ ใน YAML จะเกิดอะไรขึ้น?
หากคุณพิมพ์รหัสลับลงในไฟล์ YAML โดยตรง จะเกิดผลเสียร้ายแรงดังนี้:

ทุกคนที่เห็นโค้ดจะเห็นรหัสลับ: เนื่องจากไฟล์ YAML ถูกเก็บไว้ในโฟลเดอร์ .github/workflows ซึ่งเป็นส่วนหนึ่งของ Repository หากโปรเจกต์เป็น Public ทุกคนในโลกจะเห็นรหัสลับของคุณทันที

ประวัติการแก้ไข (Git History): แม้คุณจะลบค่าออกในภายหลัง แต่ค่ารหัสลับนั้นจะ ฝังอยู่ในประวัติ (Commit History) ตลอดไป แฮกเกอร์สามารถย้อนกลับไปดูเวอร์ชันเก่าเพื่อขโมยรหัสได้

Log รั่วไหล: เมื่อ Workflow ทำงาน ค่าที่ใส่ตรงๆ อาจถูกพ่นออกมาในหน้า Console/Log ของ GitHub Actions ทำให้คนที่มีสิทธิ์ดู Log เห็นความลับนั้นได้
...
```plaintext
# 1. ลำดับของ Git Flow ในโปรเจกต์
Develop Branch: เป็นที่รวบรวมโค้ดใหม่ๆ จาก Developer ทุกคน (เป็นด่านแรก)

Staging Branch: โค้ดที่ "คิดว่าสมบูรณ์แล้ว" จะถูกส่งมาที่นี่เพื่อทดสอบในสภาพแวดล้อมที่ จำลองเหมือนเครื่องจริงที่สุด (ใช้ทำ Smoke Tests)

Main (Production) Branch: โค้ดที่ผ่านการทดสอบจาก Staging แล้วเท่านั้นถึงจะถูกรวมเข้าที่นี่ เพื่อเปิดให้ผู้ใช้จริงใช้งาน

2. ทำไมต้องมีหลาย Environment? (ทำไมไม่ Deploy ตรง?)
การข้ามจาก develop ไป production เลยนั้นอันตรายมากครับ เหตุผลที่เราต้องแยก Environment มีดังนี้:

เพื่อความเสถียร (Stability): develop มักจะมีโค้ดที่ยังทำไม่เสร็จหรือมีบั๊กปนอยู่ การแยก staging ช่วยให้เราคัดกรองบั๊กออกก่อนที่ผู้ใช้จริงจะเจอ

การทดสอบที่สมจริง (Real-world Testing): บางครั้งโค้ดรันในเครื่องเราผ่าน (CI ผ่าน) แต่พอไปเจอ Database จริงหรือ Network จริงบน Server กลับพัง staging จึงเป็นตัวช่วยยืนยันว่าระบบ "รอด" ในสภาพแวดล้อมจริง

ลดความเสี่ยง (Risk Management): หากตรวจพบข้อผิดพลาดร้ายแรงใน staging เรายังสามารถหยุดการ Deploy ได้ทันทีโดยที่หน้าเว็บหลัก (Main) ยังใช้งานได้ปกติ ไม่ล่มตามไปด้วย

การจัดการฐานข้อมูล (Database Isolation): ช่วยแยกข้อมูลที่ใช้ทดสอบ (Test Data) ออกจากข้อมูลลูกค้าจริง (Real Data) เพื่อป้องกันการเผลอลบหรือแก้ไขข้อมูลสำคัญ
